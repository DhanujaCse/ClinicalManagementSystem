...
/*
 * @GetMapping("getreportByTestDetailsId/{TestDetailsId}") public
 * ResponseEntity<Report> printreportbyTestdetailsid(@PathVariable
 * ("TestDetailsId")int TestDetailsId ) { Report
 * cust=ser.getreportByPlacedId(TestDetailsId); if(cust!=null) {
 * logger.info("Sucessfully print all data"); return new
 * ResponseEntity<Report>(cust,HttpStatus.FOUND); } else {
 * logger.error("Try again to print Data"); return new
 * ResponseEntity<Report>(cust,HttpStatus.NOT_FOUND); } }
 */
